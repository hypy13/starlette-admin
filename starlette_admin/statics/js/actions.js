...
class ActionManager {
    ...
    initActionModal(table) {
        ...
        $("#modal-action").on("show.bs.modal", function (event) {
            ...
            if (table.query_filter && table.query_filter_count){
                submit_btn_text = `${submit_btn_text} ${table.query_filter_count} Items`
            }
            ...
            actionSubmit.on("click", function (event) {
                ...
                self.submitAction(name, form, customResponse, isRowAction, button, table);
            });
        });
    }
     ...
     */
    submitAction(actionName, form, customResponse, isRowAction, element, table) {
        let baseUrl = isRowAction ? this.rowActionUrl : this.actionUrl;
        let query = new URLSearchParams();
        query.append("name", actionName);
        console.log(table.query, table.query_count);
        if (table.query) {
            query.append("query_filter", JSON.stringify(table.query_filter));
        }
